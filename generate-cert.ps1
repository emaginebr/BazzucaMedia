...
$cert = New-SelfSignedCertificate `
    -Subject "CN=localhost, O=BazzucaMedia, C=BR" `
...
Write-Host "?? Localização: $certPath" -ForegroundColor Yellow
Write-Host "?? Senha: $CertPassword" -ForegroundColor Yellow
Write-Host ""
Write-Host "??  O certificado é válido por $DaysValid dias" -ForegroundColor Blue
Write-Host "??  Configure as variáveis de ambiente no arquivo .env:" -ForegroundColor Blue
